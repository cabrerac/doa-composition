...
def callback(ch, method, properties, body):
    ...
    time.sleep(ms)
    message_dict = {
        'req_id': req_id, 'user_topic': user_topic, 'expected_output': expected_output, 'desc': 'message from service_13_async', 'next_topic': 'service._OUTPUT_SERVICE_13',
        'parameters': [
            {'name': 'p', 'value': 'Response from service_13_async'}
        ]
    }
    next_topic = message_dict['next_topic']
    if expected_output == '_OUTPUT_SERVICE_13':
...
credentials = util.read_rabbit_credentials(rabbit_credentials_file)
consumer_thread = Consumer(credentials, 'service._OUTPUT_SERVICE_12', callback)
...
@app.route('/doa_composition/service_13_async', methods=['GET', 'POST'])
def service_13_async():
    try:
