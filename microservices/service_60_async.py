...
def callback(ch, method, properties, body):
    ...
    ms = 0.0038
    time.sleep(ms)
    description = util.read_service_description('./description/service_60.json')
    outputs = description['outputs']
    for output in outputs:
        output['value'] = 'Output value from service_60'
    message_dict = {
        'req_id': req_id, 'user_topic': user_topic, 'expected_output': expected_output, 'desc': 'message from service_60_async', 'next_topic': 'service._OUTPUT_SERVICE_60',
        'outputs': outputs
    }
    ...
    if expected_output == '_OUTPUT_SERVICE_60':
...
credentials = util.read_rabbit_credentials(rabbit_credentials_file)
consumer_thread = Consumer(credentials, 'service._OUTPUT_SERVICE_59', callback)
...
@app.route('/doa_composition/service_60_async', methods=['GET', 'POST'])
def service_60_async():
    try:
