...
def callback(ch, method, properties, body):
    ...
    ms = 0.0014
    time.sleep(ms)
    description = util.read_service_description('./description/service_4.json')
    outputs = description['outputs']
    for output in outputs:
        output['value'] = 'Output value from service_4'
    message_dict = {
...
@app.route('/doa_composition/service_4_async', methods=['GET', 'POST'])
def service_4_async():
    try:
