...
def callback(ch, method, properties, body):
    ...
    time.sleep(ms)
    message_dict = {
        'req_id': req_id, 'user_topic': user_topic, 'expected_output': expected_output, 'desc': 'message from service_38_async', 'next_topic': 'service._OUTPUT_SERVICE_38',
        'parameters': [
            {'name': 'p', 'value': 'Response from service_38_async'}
        ]
    }
