...
def callback(ch, method, properties, body):
    ...
    ms = 0.0
    time.sleep(ms)
    description = util.read_service_description('./description/service_87.json')
    outputs = description['outputs']
    for output in outputs:
        output['value'] = 'Output value from service_87'
    message_dict = {
        'req_id': req_id, 'user_topic': user_topic, 'expected_output': expected_output, 'desc': 'message from service_87_async', 'next_topic': 'service._OUTPUT_SERVICE_87',
        'outputs': outputs
    }
    ...
    if expected_output == '_OUTPUT_SERVICE_87':
...
credentials = util.read_rabbit_credentials(rabbit_credentials_file)
consumer_thread = Consumer(credentials, 'service._OUTPUT_SERVICE_86', callback)
...
@app.route('/doa_composition/service_87_async', methods=['GET', 'POST'])
def service_87_async():
    try:
