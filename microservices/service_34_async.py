...
def callback(ch, method, properties, body):
    ...
    time.sleep(ms)
    message_dict = {
        'req_id': req_id, 'user_topic': user_topic, 'expected_output': expected_output, 'desc': 'message from service_34_async', 'next_topic': 'service._OUTPUT_SERVICE_34',
        'parameters': [
            {'name': 'p', 'value': 'Response from service_34_async'}
        ]
    }
...
@app.route('/doa_composition/service_34_async', methods=['GET', 'POST'])
def service_34_async():
    try:
