...
def callback(ch, method, properties, body):
    ...
    time.sleep(ms)
    message_dict = {
        'req_id': req_id, 'user_topic': user_topic, 'expected_output': expected_output, 'desc': 'message from service_90_async', 'next_topic': 'service._OUTPUT_SERVICE_90',
        'parameters': [
            {'name': 'p', 'value': 'Response from service_90_async'}
        ]
    }
    next_topic = message_dict['next_topic']
    if expected_output == '_OUTPUT_SERVICE_90':
...
credentials = util.read_rabbit_credentials(rabbit_credentials_file)
consumer_thread = Consumer(credentials, 'service._OUTPUT_SERVICE_89', callback)
...
@app.route('/doa_composition/service_90_async', methods=['GET', 'POST'])
def service_90_async():
    try:
