...
def callback(ch, method, properties, body):
    ...
    time.sleep(ms)
    message_dict = {
        'req_id': req_id, 'user_topic': user_topic, 'expected_output': expected_output, 'desc': 'message from service_1', 'next_topic': 'service._OUTPUT_SERVICE_1',
        'parameters': [
            {'name': 'p', 'value': 'Response from service_1'}
        ]
    }
    next_topic = message_dict['next_topic']
    if expected_output == '_OUTPUT_SERVICE_1':
...
@app.route('/doa_composition/service_1', methods=['GET', 'POST'])
def service_1():
    try:
