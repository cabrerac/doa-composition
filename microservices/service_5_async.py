...
def callback(ch, method, properties, body):
    ...
    time.sleep(ms)
    message_dict = {
        'req_id': req_id, 'user_topic': user_topic, 'expected_output': expected_output, 'desc': 'message from service_5', 'next_topic': 'service._OUTPUT_SERVICE_5',
        'parameters': [
            {'name': 'p', 'value': 'Response from service_5'}
        ]
    }
...
@app.route('/doa_composition/service_5', methods=['GET', 'POST'])
def service_5():
    try:
