...
def callback(ch, method, properties, body):
    ...
    time.sleep(ms)
    message_dict = {
        'req_id': req_id, 'user_topic': user_topic, 'expected_output': expected_output, 'desc': 'message from service_93_async', 'next_topic': 'service._OUTPUT_SERVICE_93',
        'parameters': [
            {'name': 'p', 'value': 'Response from service_93_async'}
        ]
    }
    next_topic = message_dict['next_topic']
    if expected_output == '_OUTPUT_SERVICE_93':
...
credentials = util.read_rabbit_credentials(rabbit_credentials_file)
consumer_thread = Consumer(credentials, 'service._OUTPUT_SERVICE_92', callback)
...
@app.route('/doa_composition/service_93_async', methods=['GET', 'POST'])
def service_93_async():
    try:
