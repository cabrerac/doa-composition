...
def callback(ch, method, properties, body):
    ...
    time.sleep(ms)
    message_dict = {
        'req_id': req_id, 'user_topic': user_topic, 'expected_output': expected_output, 'desc': 'message from service_36_async', 'next_topic': 'service._OUTPUT_SERVICE_36',
        'parameters': [
            {'name': 'p', 'value': 'Response from service_36_async'}
        ]
    }
