...
def callback(ch, method, properties, body):
    ...
    time.sleep(ms)
    message_dict = {
        'req_id': req_id, 'user_topic': user_topic, 'expected_output': expected_output, 'desc': 'message from service_77', 'next_topic': 'service._OUTPUT_SERVICE_77',
        'parameters': [
            {'name': 'p', 'value': 'Response from service_77'}
        ]
    }
...
@app.route('/doa_composition/service_77', methods=['GET', 'POST'])
def service_77():
    try:
