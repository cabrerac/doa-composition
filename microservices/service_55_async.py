...
def callback(ch, method, properties, body):
    ...
    time.sleep(ms)
    message_dict = {
        'req_id': req_id, 'user_topic': user_topic, 'expected_output': expected_output, 'desc': 'message from service_55', 'next_topic': 'service._OUTPUT_SERVICE_55',
        'parameters': [
            {'name': 'p', 'value': 'Response from service_55'}
        ]
    }
...
@app.route('/doa_composition/service_55', methods=['GET', 'POST'])
def service_55():
    try:
