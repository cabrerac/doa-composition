...
def callback(ch, method, properties, body):
    ...
    ms = 0.0046
    time.sleep(ms)
    description = util.read_service_description('./description/service_55.json')
    outputs = description['outputs']
    for output in outputs:
        output['value'] = 'Output value from service_55'
    message_dict = {
        'req_id': req_id, 'user_topic': user_topic, 'expected_output': expected_output, 'desc': 'message from service_55_async', 'next_topic': 'service._OUTPUT_SERVICE_55',
        'outputs': outputs
    }
    ...
    if expected_output == '_OUTPUT_SERVICE_55':
...
credentials = util.read_rabbit_credentials(rabbit_credentials_file)
consumer_thread = Consumer(credentials, 'service._OUTPUT_SERVICE_54', callback)
...
@app.route('/doa_composition/service_55_async', methods=['GET', 'POST'])
def service_55_async():
    try:
