...
def callback(ch, method, properties, body):
    ...
    time.sleep(ms)
    message_dict = {
        'req_id': req_id, 'user_topic': user_topic, 'expected_output': expected_output, 'desc': 'message from service_31', 'next_topic': 'service._OUTPUT_SERVICE_31',
        'parameters': [
            {'name': 'p', 'value': 'Response from service_31'}
        ]
    }
...
@app.route('/doa_composition/service_31', methods=['GET', 'POST'])
def service_31():
    try:
