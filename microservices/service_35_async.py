...
def callback(ch, method, properties, body):
    ...
    time.sleep(ms)
    message_dict = {
        'req_id': req_id, 'user_topic': user_topic, 'expected_output': expected_output, 'desc': 'message from service_35', 'next_topic': 'service._OUTPUT_SERVICE_35',
        'parameters': [
            {'name': 'p', 'value': 'Response from service_35'}
        ]
    }
...
@app.route('/doa_composition/service_35', methods=['GET', 'POST'])
def service_35():
    try:
