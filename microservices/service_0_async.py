...
description = util.read_service_description('./description/service_0.json')
requests = {}
...
def callback(ch, method, properties, body):
    ...
    try:
        ...
        if util.compare(description['inputs'], inputs):
            ...
            user_topic = message['user_topic']
            message_dict = {
                    'req_id': req_id, 'user_topic': user_topic, 'desc': 'message from ' + description['name'] + '_async ::: else', 'next_topic': user_topic
            }
            credentials = util.read_rabbit_credentials(rabbit_credentials_file)
            producer = Producer(credentials)
            producer.publish(user_topic, message_dict)
...
@app.route('/doa_composition/service_0_async', methods=['GET', 'POST'])
def service_0_async():
    try:
