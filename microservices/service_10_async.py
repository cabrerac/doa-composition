...
def callback(ch, method, properties, body):
    ...
    time.sleep(ms)
    message_dict = {
        'req_id': req_id, 'user_topic': user_topic, 'expected_output': expected_output, 'desc': 'message from service_10', 'next_topic': 'service._OUTPUT_SERVICE_10',
        'parameters': [
            {'name': 'p', 'value': 'Response from service_10'}
        ]
    }
...
credentials = util.read_rabbit_credentials(rabbit_credentials_file)
consumer_thread = Consumer(credentials, 'service._OUTPUT_SERVICE_9', callback)
...
@app.route('/doa_composition/service_10', methods=['GET', 'POST'])
def service_10():
    try:
