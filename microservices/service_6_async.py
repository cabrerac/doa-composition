...
def callback(ch, method, properties, body):
    ...
    time.sleep(ms)
    message_dict = {
        'req_id': req_id, 'user_topic': user_topic, 'expected_output': expected_output, 'desc': 'message from service_6', 'next_topic': 'service._OUTPUT_SERVICE_6',
        'parameters': [
            {'name': 'p', 'value': 'Response from service_6'}
        ]
    }
...
@app.route('/doa_composition/service_6', methods=['GET', 'POST'])
def service_6():
    try:
