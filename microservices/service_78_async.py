...
def callback(ch, method, properties, body):
    ...
    ms = 0.0053
    time.sleep(ms)
    description = util.read_service_description('./description/service_78.json')
    outputs = description['outputs']
    for output in outputs:
        output['value'] = 'Output value from service_78'
    message_dict = {
        'req_id': req_id, 'user_topic': user_topic, 'expected_output': expected_output, 'desc': 'message from service_78_async', 'next_topic': 'service._OUTPUT_SERVICE_78',
        'outputs': outputs
    }
    ...
    if expected_output == '_OUTPUT_SERVICE_78':
...
credentials = util.read_rabbit_credentials(rabbit_credentials_file)
consumer_thread = Consumer(credentials, 'service._OUTPUT_SERVICE_77', callback)
...
@app.route('/doa_composition/service_78_async', methods=['GET', 'POST'])
def service_78_async():
    try:
