...
def callback(ch, method, properties, body):
    ...
    time.sleep(ms)
    message_dict = {
        'req_id': req_id, 'user_topic': user_topic, 'expected_output': expected_output, 'desc': 'message from service_73', 'next_topic': 'service._OUTPUT_SERVICE_73',
        'parameters': [
            {'name': 'p', 'value': 'Response from service_73'}
        ]
    }
...
@app.route('/doa_composition/service_73', methods=['GET', 'POST'])
def service_73():
    try:
