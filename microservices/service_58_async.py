...
def callback(ch, method, properties, body):
    ...
    ms = 0.0039
    time.sleep(ms)
    description = util.read_service_description('./description/service_58.json')
    outputs = description['outputs']
    for output in outputs:
        output['value'] = 'Output value from service_58'
    message_dict = {
        'req_id': req_id, 'user_topic': user_topic, 'expected_output': expected_output, 'desc': 'message from service_58_async', 'next_topic': 'service._OUTPUT_SERVICE_58',
        'outputs': outputs
    }
    ...
    if expected_output == '_OUTPUT_SERVICE_58':
...
credentials = util.read_rabbit_credentials(rabbit_credentials_file)
consumer_thread = Consumer(credentials, 'service._OUTPUT_SERVICE_57', callback)
...
@app.route('/doa_composition/service_58_async', methods=['GET', 'POST'])
def service_58_async():
    try:
